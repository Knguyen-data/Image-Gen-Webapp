...
interface NavItem {
  ...
  permission?: 'users:read' | 'analytics:read' | 'system:read' | 'content:moderate' | 'settings:read';
}
...
export const AdminSidebar: React.FC = () => {
  ...
  const filteredNavItems = navItems.filter(item => {
    ...
    return hasPermission(adminUser, item.permission);
  });
    ...
    <aside className="fixed left-0 top-0 h-full w-64 bg-gray-900 border-r border-gray-800 flex flex-col">
      ...
      <div className="p-6 border-b border-gray-800">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-dash-500 rounded-lg flex items-center justify-center">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                isActive
                  ? 'bg-dash-500/10 text-dash-400 border border-dash-500/20'
